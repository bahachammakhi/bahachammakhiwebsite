...
export const SEO = ({
  ...
  const structuredDataArticle = `{
		...
		"image": "${
      cover
        ? `https://www.bahachammakhi.tn${cover}`
        : `https://res.cloudinary.com/bahachammakhi/image/upload/v1613310888/Black_Technology_Blog_Banner_ea809724f2.png`
    }",
		...
		"url": "${url}${location}/?ref=www.bahachammakhi.tn"
	}`;
  ...
  return (
    <Helmet>
      ...
      <meta
        property="og:url"
        content={`${url}${location}/?ref=www.bahachammakhi.tn`}
      />
      <meta
        property="og:type"
      ...
      <meta
        property="og:title"
        content={title ? `Baha | ${title}` : defaultTitle}
      />
      <meta
      ...
      <title>{title ? `Baha | ${title}` : defaultTitle}</title>
      {type === "NewsArticle" && (
